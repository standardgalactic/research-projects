...
Requests are batched at exactly 20 accounts per query; at the time of the
preliminary snapshot reported here, 1,920 GraphQL batches had been completed,
yielding $\sim31{,}900$ account records. The total follow list spans
approximately 1,000,800 accounts distributed across 50,040 batches. Each batch
retrieves a page of followed accounts together with the metadata fields listed in
Section~2. Results are written incrementally to three output files:
\texttt{following-activity.csv} (the primary structured record),
\texttt{active-following.txt}, and \texttt{inactive-following.txt}.
...
\paragraph{\texttt{analyse\_activity.py}} This script computes the overall
active/inactive split and the per-language activity ratio $R$. Languages are
filtered to those with at least 100 total accounts before sorting by total
presence. It also constructs a year-resolution survival table by binning account
ages (derived from \texttt{created\_at}) and computing the active fraction within
each bin, from which the approximate half-life is read off as the first bin whose
active fraction falls below $0.5$.

\paragraph{\texttt{analyse\_languages.py}} This script separately tabulates the
top languages by count among active accounts and among inactive accounts,
reporting each language's share of its respective population. This decomposition
exposes compositional differences between the active and inactive subpopulations
that are not visible in the combined ratio alone.

\paragraph{\texttt{language\_survival.py}} This script computes median developer
lifetimes per language, defined as the median of the set
$\{(\texttt{last\_pushed\_at} - \texttt{created\_at}) / 365.25\}$ over all
accounts with both timestamps present. Languages with fewer than 20 observations
are excluded. Results are sorted in descending order of median lifetime.

\subsection{Rate Limiting and Projected Completion}

At 20 accounts per GraphQL batch, the full follow list of approximately
1,000,800 accounts is distributed across exactly 50,040 batches. GitHub imposes
a rate limit of 5,000 GraphQL points per hour on authenticated requests; the
effective crawl rate is therefore determined by the point cost of each batch
query. The crawler is designed to run continuously as a background process,
writing results incrementally so that intermediate analyses can be re-run at any
point against the partial dataset without interrupting collection. Each
intermediate run constitutes a consistent snapshot: the active/inactive
classification threshold is applied uniformly at analysis time against the
\texttt{last\_pushed\_at} field, so the definition of activity does not drift
across snapshots.
