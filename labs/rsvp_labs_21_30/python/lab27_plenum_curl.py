...
Then call `run()` to write frames into data/lab27_plenum_curl/.
...
def run(output_dir: str = f"data/lab27_plenum_curl", n_steps: int = 100) -> None:
    out = Path(output_dir)
    ...
    with open(out / "frame_000.json", "w") as f:
        json.dump(dummy, f)
    print(f"Lab 27 placeholder frame written to {out / 'frame_000.json'}")
