...
Then call `run()` to write frames into data/lab21_morphogen_mosaic/.
...
def run(output_dir: str = f"data/lab21_morphogen_mosaic", n_steps: int = 100) -> None:
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    # TODO: replace with actual simulation loop
    dummy = {
        "lab": 21,
        "title": "Morphogen Mosaic",
        "note": "placeholder frame; implement simulation to populate this structure"
    }
    with open(out / "frame_000.json", "w") as f:
        json.dump(dummy, f)
    print(f"Lab 21 placeholder frame written to {out / 'frame_000.json'}")
