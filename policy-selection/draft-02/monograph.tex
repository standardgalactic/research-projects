...
- $\lambda < 0.3$: strong exploratory activation ($|a_2^*| \gg 0$),
- $0.3 < \lambda < 0.47$: weak but persistent probing,
- $\lambda > 0.47$: collapse to null policy.
...
Results:
- Without epistemic drive: null policy selected ($\lambda > \lambda^*_{\text{risk}} \approx 0.5$).
- With epistemic bonus: central atom activated twice, propagating information forward; final $o_T \approx 0.95$, uncertainty reduced by 60%.
- Total active controls: 2 out of 15 possible (86% sparsity).
...
\section{Policy Binding via Variational Message Passing}
...
\section{Conclusions: Horizons for Sparse Bayesian Cognition}
...
\item \textbf{Control-theoretic extensions}: Derive optimal $\lambda(t), \gamma(t)$ schedules via meta-inference over cognitive regimes, enabling adaptive sparsity for artificial agents.
...
The sparse Bayesian RSVP framework generates precise, falsifiable predictions for neural implementation. Below is a systematic mapping from theoretical constructs to known brain systems and measurable signals.

\begin{enumerate}
\item \textbf{Policy atoms ($\psi_k$)}:
   \begin{itemize}
   \item \emph{Location}: Hippocampal place/grid cells, entorhinal cortical modules.
   \item \emph{Signature}: Sharp-wave ripple sequences encoding trajectory fragments.
   \item \emph{Prediction}: Atom activation correlates with theta-phase precession during planning.
   \end{itemize}

\item \textbf{Sparsity parameters ($\lambda_1, \lambda_2$)}:
   \begin{itemize}
   \item \emph{Location}: Neuromodulatory control (norepinephrine, acetylcholine).
   \item \emph{Signature}: LC-NE burst amplitude scales with cognitive load; high $\lambda$ → suppressed ripple density.
   \item \emph{Prediction}: Pharmacological blockade of noradrenergic input increases policy beam width.
   \end{itemize}

\item \textbf{Binding strength ($\beta_{jk}$)}:
   \begin{itemize}
   \item \emph{Location}: Gamma-band synchrony between cortical columns.
   \item \emph{Signature}: Cross-frequency coupling (theta--gamma) during concept formation.
   \item \emph{Prediction}: Binding bifurcation detectable via sudden onset of coherent gamma in overlapping assemblies.
   \end{itemize}

\item \textbf{Self-template ($\mathcal{B}_{\text{self}}$)}:
   \begin{itemize}
   \item \emph{Location}: Default mode network (DMN), medial prefrontal cortex.
   \item \emph{Signature}: Persistent low-frequency fluctuations correlated across self-referential tasks.
   \item \emph{Prediction}: DMN disruption (via TMS) lowers $\gamma$, inducing transient ego dissolution.
   \end{itemize}

\item \textbf{Criticality ($\gamma \approx \gamma^*$)}:
   \begin{itemize}
   \item \emph{Location}: Whole-brain avalanche dynamics.
   \item \emph{Signature}: Power-law distributed neuronal avalanches ($P(s) \propto s^{-\alpha}$, $\alpha \approx 1.5$).
   \item \emph{Prediction}: Flow states show maximal deviation from $1/f$ toward critical exponent.
   \end{itemize}
\end{enumerate}
...
Start from the nonlinear entropy production term $\sigma (\nabla \Phi)^2$. Under co-activation $a_1 \psi_1 + a_2 \psi_2$, the gradient is:

\[
\nabla \Phi = a_1 \nabla \psi_1 + a_2 \nabla \psi_2 + \beta_{12} \nabla \mathcal{I}_{12}.
\]

The binding energy contribution is:

\[
\Delta S = \sigma \int (\beta_{12} \nabla \mathcal{I}_{12})^2 \, dx - \eta \beta_{12} \int \mathcal{I}_{12} \, dx.
\]

Minimizing w.r.t. $\beta_{12}$ with $\ell_1$ cost yields the threshold:

\[
\lambda^*_{\text{bind}} = \frac{\sigma \alpha^2 \|\nabla (\psi_1 + \psi_2)\|^2}{2\eta},
\]

derived via subgradient stationarity and overlap integral evaluation.
