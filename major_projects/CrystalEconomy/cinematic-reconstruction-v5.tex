...
\subsection{The Pneumatic Model and Its Limitations}

A common implicit model of character emotion in generative systems treats affective state as a scalar or categorical label: a hidden variable in a finite set $\{\text{happy}, \text{angry}, \text{sad}, \text{fearful}, \ldots\}$ that the rendering system reads out and expresses. In this model, affect accumulates, triggers expression, and discharges:

\[
\text{emotion accumulates} \;\to\; \text{expression} \;\to\; \text{discharge}.
\]

Call this the \textbf{pneumatic model}, by analogy with a hydraulic system in which pressure builds, is released through a valve, and the system returns to its prior state. The pneumatic model is implicit in any architecture that treats affect as a pre-existing hidden variable that the animation or dialogue system merely reveals.

The pneumatic model has several properties that are convenient for implementation and inconvenient for characterization. Affect is discrete and recoverable: once an emotion is expressed, the internal state returns to its prior level, and the character is available for the next emotional assignment. Affect is compositionally independent of action: the character's internal state is set externally, and behavior is a function of that state, not a participant in the construction of subsequent states. And affect is injectively legible: a sad-looking face is evidence of sadness, and the appropriate response to observing it is to read off sadness as the committed internal state.

Each of these properties is architecturally problematic. The first prevents hysteresis and genuine character development. The second prevents expression from being constitutive of subsequent affect. The third leads directly to an inverse fallacy that the framework's admissible-reconstruction apparatus is specifically designed to avoid.

\subsection{The Coupled Character State}

\begin{definition}[Coupled Character State]
For a character $c$ in the persistent world, define the \textbf{coupled character state} at time $t$ as the tuple
\[
X_t^{(c)} = (B_t,\; A_t,\; M_t,\; Q_t,\; R_t,\; K_t),
\]
where:
\begin{itemize}[leftmargin=2em,itemsep=0pt]
\item $B_t$: bodily configuration (posture, muscular tension, facial configuration, respiration, motor activation);
\item $A_t$: currently activated associations and attentional focus;
\item $M_t$: accessible memories, weighted by current cue-compatibility;
\item $Q_t$: appraisal of current context (threat evaluation, relevance assessment, agency attribution);
\item $R_t$: social relations and current responses of other agents in the scene;
\item $K_t$: established character dispositions, commitments, and history constraints accumulated in $H_t$.
\end{itemize}
\end{definition}

\begin{definition}[Affective Dynamics]
The character state evolves according to
\[
X_{t+1} = F(X_t,\; \mathcal{W}_t,\; a_t,\; o_t),
\]
where $\mathcal{W}_t$ is the current world state, $a_t$ is the action or expression the character produces at time $t$, and $o_t$ is the observation of responses from other agents and the environment. The function $F$ is the character's coupled dynamics: it integrates the character's own actions, physiological changes, memory retrieval, contextual reappraisal, and social feedback into the subsequent state.
\end{definition}

The crucial architectural departure from the pneumatic model is that $a_t$ appears as an input to $F$. Expression is not merely a read-out of $X_t$; it is a cause of $X_{t+1}$. If a character clenches their fists, begins pacing, and raises their voice, those actions change bodily configuration ($B_{t+1}$), activate associated memories and appraisals ($A_{t+1}$, $Q_{t+1}$), and elicit responses from other agents ($R_{t+1}$) that feed back into the subsequent state:

\[
X_t \;\xrightarrow{\;\text{clench, pace}\;}\; X_{t+1} \;\xrightarrow{\;\text{hostile response}\;}\; X_{t+2} \;\xrightarrow{\;\text{associated recollection}\;}\; X_{t+3}.
\]

An expressive act is therefore constitutive of subsequent affective state rather than merely evidentiary of a pre-existing affective state. This is a behavioral and dynamical claim, not a claim about any specific neurochemical mechanism, and it is sufficient to derive the consequences below.

\begin{remark}
The coupled-state formulation is consistent with a range of constructivist accounts of emotion in behavioral science. The present framework does not require any particular theory of the underlying causal mechanisms; it requires only that the dynamics $F$ be such that expressions and actions enter as inputs rather than purely as outputs. Whether $F$ is a learned neural model, a symbolic rule system, a physics-informed simulation of muscular and postural dynamics, or a hybrid is an implementation choice left to the Algorithm Registry.
\end{remark}

\subsection{Character Affect Non-Identifiability}

The coupled character state implies a non-identifiability result that applies the admissible-reconstruction argument directly to characterization.

\begin{definition}[Behavioral Observation Map]
Let
\[
O : \mathcal{X} \to \mathcal{Y}
\]
be the map from latent character states to observable behavior and expression. The observable $y = O(x)$ encompasses facial configuration, posture, voice, action, and any other externally measurable output.
\end{definition}

\begin{proposition}[Character Affect Non-Identifiability]
\label{prop:affect-nonid}
For any behavioral observation $y \in \mathcal{Y}$, the set of latent character states consistent with $y$ is generically non-trivial:
\[
[x]_O = \{x' \in \mathcal{X} : O(x') = O(x)\} \;\neq\; \{x\}.
\]
Consequently:
\[
\text{smiling} \not\Rightarrow \text{happiness}, \qquad \text{crying} \not\Rightarrow \text{sadness}, \qquad O(x) = y \not\Rightarrow x = O^{-1}(y),
\]
unless $O$ is injective on the relevant region of $\mathcal{X}$.
\end{proposition}

\begin{proof}[Justification]
Distinct internal states can produce observationally equivalent behavior. A character producing a grief expression ($O(x) = y_{\mathrm{grief}}$) may be in a state of genuine grief, deliberately performing grief to deceive another character, experiencing pain misidentified by observers as grief, exhausted and producing expressions that resemble grief by motor coincidence, or operating within a social convention that calls for grief expression independent of internal state. All of these are distinct elements of $[x]_O$, and distinguishing among them requires evidence beyond the observation $y$ alone.
\end{proof}

\begin{definition}[Character State Equivalence Class]
The \textbf{admissible character state class} at time $t$ given observation $y_t$ and history $H_t$ is
\[
\mathcal{R}_c(y_t, H_t) = \{x \in \mathcal{X} : O(x) = y_t,\; x \text{ is consistent with } H_t\}.
\]
\end{definition}

This is directly parallel to the admissible reconstruction set for world state (Definition~\ref{sec:admissible}.3). Just as an observed frame $I_t$ determines an equivalence class of possible world states rather than a unique world state, an observed behavioral expression $y_t$ determines an equivalence class of possible character states rather than a unique internal state. Subsequent observations narrow the class; committed world events constrain it.

\begin{proposal}
The world state $\mathcal{W}_t$ should represent each character's affective state not as a committed discrete label but as a distribution over the admissible class $\mathcal{R}_c(y_{0:t}, H_t)$. A grieving-looking face at time $t$ is an observation; the internal state of the character is a latent to be constrained by accumulated evidence rather than immediately committed as a categorical label. This makes characters substantially more interesting to audiences and to the story engine, because the world model and the audience need not possess the same information about character interiority.
\end{proposal}

\begin{remark}
A conventional generative system that treats behavioral output as sufficient evidence for internal state commits the inverse fallacy: it generates a sad-looking face and implicitly promotes sadness to committed world state. The architecture prevents this promotion. The observation establishes the equivalence class; only accumulated evidence (the character's subsequent behavior, revealed intentions, other characters' interpretations, causal consequences) narrows the class toward a commitment.
\end{remark}

\subsection{Hysteresis and Character Development}

The dynamical character of $F$ implies hysteresis: the character's internal state depends on the path through prior states, not merely on the current external conditions.

\begin{proposition}[Character Hysteresis]
\label{prop:hysteresis}
For the dynamics $X_{t+1} = F(X_t, \mathcal{W}_t, a_t, o_t)$, the recovery of prior world conditions does not in general recover prior character state:
\[
\mathcal{W}_{t+k} \approx \mathcal{W}_t \;\not\Rightarrow\; X_{t+k}^{(c)} \approx X_t^{(c)}.
\]
\end{proposition}

\begin{proof}[Justification]
The dynamics $F$ take the history of actions $a_{t:t+k}$ and observations $o_{t:t+k}$ as inputs at each step. Even if the external world state returns to approximately $\mathcal{W}_t$, the character's accessible memories $M_{t+k}$, established character history $K_{t+k}$, and modified social relations $R_{t+k}$ will differ from $M_t, K_t, R_t$ because the intervening episode has been added to $H_{t+k}$.
\end{proof}

This is character development in formal terms. A sequence of interactions, provocations, losses, discoveries, or expressions changes the character's state in ways that persist even after the immediate cause is removed. An argument committed to $H_t$ is part of the history that constrains all future character states. The player of an operator-controlled movie who repeatedly changes the character's circumstances will find that the character's responses gradually shift, not because a discrete character-arc variable has been incremented, but because the coupled dynamical system has accumulated evidence and altered its own subsequent behavior.

\begin{hypothesis}
\label{hyp:hysteresis}
A character model using the coupled dynamics $F$ will produce measurably more path-dependent responses to repeated operator perturbations than a character model using a memoryless discrete emotional-label assignment, as measured by variance across repeated applications of the same operator sequence from the same external world state.
\end{hypothesis}

\subsection{Operators as Perturbations to Character Dynamics}

Operators in the interactive language (Section~\ref{sec:operators}) acquire additional precision in the coupled-state framework. An instruction such as \textsc{ANGRIER} is not a style slider adjusting the readout of a pre-existing affective variable. It is an operator applied to the character dynamics:

\[
\Omega_{\mathrm{ANGRIER}} : F \;\to\; F',
\]

where $F'$ is a modified dynamics function that, for the same current state $X_t$ and world state $\mathcal{W}_t$, produces higher-tension continuations across all coupled state components. The operator may do this by modifying the appraisal component $Q_t$ (increasing attributed threat or disrespect), the bodily activation component $B_t$ (increasing muscular tension and respiratory rate), the memory accessibility $M_t$ (increasing retrieval weight for anger-compatible memories), or the social sensitivity $R_t$ (increasing the response magnitude to other agents' cues).

More complex operators now have clear behavioral mechanisms:

\begin{example}
The operator sequence \textsc{NOIR} \textsc{SUSPICIOUS} \textsc{FOLLOW MARA} \textsc{PROVOKE} \textsc{DRY} affects five distinct system components. \textsc{NOIR} modifies the genre grammar $G_t$ and the style operator $S_\theta$. \textsc{SUSPICIOUS} modifies the appraisal component $Q_t$ of relevant characters, increasing threat attribution and reducing trust weighting in $R_t$. \textsc{FOLLOW MARA} changes the camera policy $C_t$. \textsc{PROVOKE} commits an event to $H_t$ that changes the social observation $o_t$ for the targeted character. \textsc{DRY} modifies the dynamics $F$ in the manner described in the next subsection. The resulting affective trajectory is generated by the changed system, not painted onto the characters after generation.
\end{example}

\subsection{The Dry Response as Dynamical Loop Interruption}

A particularly interesting operator is the dry or incongruous response to provocation. Under the coupled dynamics, a standard escalation sequence follows a feedback loop:

\[
\text{provocation} \;\to\; \text{anger expression} \;\to\; \text{reciprocal anger from other agents} \;\to\; \text{increased threat appraisal} \;\to\; \cdots
\]

Each step reinforces the next through the feedback structure of $F$: heightened bodily tension increases threat appraisal, reciprocal anger increases social hostility scores in $R_t$, and the loop amplifies.

A dry or incongruous response interrupts this loop by substituting an expression that is incompatible with the escalating dynamics:

\[
\text{provocation} \;\to\; \text{dry incongruity} \;\to\; \text{changed appraisal in all parties} \;\to\; \text{different continuation}.
\]

The dry response changes the social observation $o_t$ for all agents in the scene, alters the accessible memory distribution $M_t$ (making incongruity-compatible recollections more available), and modifies the appraisal $Q_t$ by defusing the threat attribution. The continuation is structurally different from the escalation path, not because a different emotion label was assigned, but because the dynamics entered a different basin of attraction.

\begin{proposal}
The \textsc{DRY} operator is formally a basin-switching operator on the character dynamics: it modifies $F$ to reduce the feedback gain of the escalation loop, making the high-tension attractor less stable and the incongruity attractor more accessible. This gives the interactive operator language a mechanism for tonal control that operates through the dynamics rather than through post-hoc style adjustment.
\end{proposal}

\subsection{Integration with the Admissible World Architecture}

The constructed-affect framework integrates with the broader architecture at several points.

The admissible reconstruction machinery handles what can be inferred about characters from behavior: observations of expression constrain the character state equivalence class $\mathcal{R}_c(y_{0:t}, H_t)$ rather than committing a discrete label. The coupled dynamics $F$ handle how character states evolve in response to committed events, the character's own expressions, and the responses of other agents. The operator language perturbs those dynamics via genre-grammar transformation, appraisal modification, and basin-switching operators. The process grammar (Section~\ref{sec:grammars}) provides the relational context within which behavioral anomalies are detected: a character's behavior may be locally plausible but relationally inconsistent with the character's established $K_t$ and $H_t$. And the persistent world ensures that consequences of character expression accumulate in $H_t$ rather than disappearing when the next frame is generated.

\begin{hypothesis}
\label{hyp:affect-integration}
A world model maintaining coupled character state $X_t$ and admissible character state classes $\mathcal{R}_c(y_{0:t}, H_t)$ will produce narrative continuations exhibiting greater internal consistency (measured by human evaluation of behavioral plausibility given character history) than a system assigning discrete emotional labels directly as committed world state.
\end{hypothesis}

\begin{remark}
The section has deliberately avoided specifying any particular computational implementation of $F$. The coupled dynamics may be realized as a learned recurrent model conditioned on character history, a symbolic rule system deriving state updates from committed narrative events, a physics simulation of postural and muscular dynamics, or a combination. The architectural commitment is only to the interface: $F$ takes the full coupled state, the world state, the character's own action, and the observations from other agents as inputs, and it produces a new coupled state as output. The expression $a_t$ is an input, not only an output.
\end{remark}
